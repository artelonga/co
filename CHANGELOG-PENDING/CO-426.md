...
- Auth: all five endpoints require a vault token or JWT (same scheme as the vault/sync/
  agent-session writes); the Gestão *panel* surface is admin-only (the `/gestao` page is
  served only to admin emails). This keeps the `summary` endpoint reachable by the
  token-authenticated CO-427 launcher consumer.
