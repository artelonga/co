...
## Status: canonical post-GitHub spec (as of 2026-04-26)

This roadmap **supersedes** the universe-as-repo / GitHub-clone approach (CO-50, CO-55).
On 2026-04-18 the v2.0 direction was set: GitHub becomes an optional export destination,
not a required input. All client-server sync flows in this document use the **Vault REST
API** (CO-35, deployed) — no SSH, no deploy keys, no `git clone` on the server.

The five phases below are the post-GitHub plan ("INFRA-1..6" in earlier notes):

| Phase | a.k.a. | Status |
|-------|--------|--------|
| 1. CLI sync (`co sync`) | INFRA-1 (auth + sync API client) | todo (CO-51) |
| 2. Desktop tray | INFRA-2 (always-on agent) | todo (CO-58) |
| 3. Obsidian deep | INFRA-3 (already exists, needs enhancements) | partial (CO-34 done) |
| 4. PWA offline | INFRA-4 (browser-side queue) | todo |
| 5. Mobile sync | INFRA-5 (Capacitor) | todo |
| Sync protocol v1 | INFRA-6 (op log + 3-way merge) | todo (CO-61, CO-62) |

GitHub-related tasks deprecated: **CO-50** (universe-as-repo), **CO-55** (GitHub SSH auth).
The deploy-key fallback in `co-web/src/git_sync.rs` is dead code on the post-GitHub path
and should be removed once Phase 1 ships.
