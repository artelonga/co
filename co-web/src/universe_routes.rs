...
// ---------------------------------------------------------------------------
// CO-50: Git-backed universe handlers
// ---------------------------------------------------------------------------

/// GET /api/v1/universes/:slug/git — return current git config for a universe.
pub async fn get_universe_git(
    State(state): State<AppState>,
    Path(slug): Path<String>,
) -> Result<impl IntoResponse, AppError> {
    let storage = lock_storage(&state)?;
    storage
        .get_universe(&slug)
        .ok_or_else(|| AppError::NotFound(format!("Universe '{}' not found", slug)))?;
    match storage.get_universe_git_config(&slug) {
        Some(cfg) => Ok(Json(cfg).into_response()),
        None => Ok(StatusCode::NO_CONTENT.into_response()),
    }
}

/// PUT /api/v1/universes/:slug/git — configure the git repo for a universe (owner only).
pub async fn set_universe_git(
    State(state): State<AppState>,
    Path(slug): Path<String>,
    user_id: crate::auth::UserId,
    Json(body): Json<crate::models::SetGitConfig>,
) -> Result<impl IntoResponse, AppError> {
    {
        let storage = lock_storage(&state)?;
        let universe = storage
...

    {
        let mut storage = lock_storage(&state)?;
        storage
            .set_universe_git_config(&slug, &body.repo, body.path.as_deref(), &body.branch)
            .map_err(|e| AppError::Internal(e.to_string()))?;
    }
...
/// POST /api/v1/universes/:slug/sync — trigger a manual sync (owner/admin only).
///
/// Clones the repo if needed, then pulls + re-indexes entries.
...
pub async fn sync_universe(
    ...
    // Authorise: must be owner.
    let (git_config, data_dir) = {
        let storage = lock_storage(&state)?;
        let universe = storage
            .get_universe(&slug)
            .ok_or_else(|| AppError::NotFound(format!("Universe '{}' not found", slug)))?;
        if universe.owner_id != user_id.0 {
            return Err(AppError::Forbidden(
                "Only the owner can trigger a manual sync".into(),
            ));
        }
        let git = storage
            .get_universe_git_config(&slug)
            .ok_or_else(|| AppError::BadRequest("Universe has no git repo configured".into()))?;
        let dir = storage.data_dir.clone();
        (git, dir)
    };

    do_git_sync(state, slug, git_config, data_dir, true).await
}

/// POST /api/v1/universes/:slug/webhook — GitHub push webhook → immediate re-sync.
///
/// Verifies that the push is to the tracked branch and triggers a sync.
/// No auth required (GitHub does not send our JWT).
pub async fn webhook_sync(
    State(state): State<AppState>,
    Path(slug): Path<String>,
    Json(payload): Json<crate::models::GithubWebhookPush>,
) -> Result<axum::response::Response, AppError> {
    let (git_config, data_dir) = {
        let storage = lock_storage(&state)?;
        // Universe must exist and have a git repo configured.
        storage
            .get_universe(&slug)
            .ok_or_else(|| AppError::NotFound(format!("Universe '{}' not found", slug)))?;
        let git = storage
    ...
    let expected_ref = format!("refs/heads/{}", git_config.branch);
    if payload.git_ref != expected_ref {
        return Ok(Json(serde_json::json!({
            "status": "skipped",
            "reason": "push is not to the tracked branch"
        }))
        .into_response());
    }

    do_git_sync(state, slug, git_config, data_dir, false).await
}

/// Shared sync implementation: runs git operations in a blocking thread,
/// then updates the entry index and commit hash in the database.
///
/// `force`: if true, always pull regardless of the last sync time.
async fn do_git_sync(
    state: AppState,
    slug: String,
    git_config: crate::models::UniverseGitConfig,
    ...
    let synced_at = git_config.synced_at.map(|dt| dt.to_rfc3339());

    // Skip if recently synced and not forced.
    if !force && !git_sync::needs_lazy_sync(synced_at.as_deref()) {
        return Ok(Json(serde_json::json!({
            "status": "cached",
            "commit_hash": known_hash
        }))
        .into_response());
    }
    ...
    // Run blocking git operations off the async thread pool.
    ...
    match sync_result {
        Ok(None) => {
            // Already up to date — refresh synced_at so we don't check again for 5 min.
            ...
            Ok(Json(serde_json::json!({
                "status": "up_to_date",
                "commit_hash": git_config.commit_hash
            }))
            .into_response())
        }
        Ok(Some((new_hash, entries))) => {
            // Persist entries + new commit hash.
            let indexed = {
                let mut storage = state
                    ...
                    .map_err(|_| AppError::Internal("Storage lock failed".into()))?;
                let n = storage
                    .upsert_entries_from_sync(&slug, &entries)
                    .map_err(|e| AppError::Internal(e.to_string()))?;
                storage
                    .update_universe_git_sync(&slug, &new_hash)
                    .map_err(|e| AppError::Internal(e.to_string()))?;
                n
            };
            Ok(Json(serde_json::json!({
        ...
        Err(e) => {
            // Record the error so it's visible, but don't crash the request.
            if let Ok(mut storage) = state.storage.lock() {
                let _ = storage.set_universe_git_error(&slug, &e.to_string());
            }
            Err(AppError::Internal(format!("Git sync failed: {}", e)))
        }
    }
}
...
pub fn router() -> Router<AppState> {
    ...
    let protected_routes = Router::new()
        ...
        // CO-50: git config write + manual sync (auth required)
        .route("/{slug}/git", put(set_universe_git))
        .route("/{slug}/sync", post(sync_universe))
