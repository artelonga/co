...
pub async fn apply_template_all(
    ...
    for universe in &owned {
        ...
        for (rel, body_text) in &scaffold {
            ...
            if co::write_entry(&universe_root, &entry).is_ok() {
                ...
                let _ =
                    crate::repository::SqliteEntryRepository::new(uc).upsert_entry(slug, &entry);
                {
    ...
    let hub_entry_path = if !hub_slug.is_empty() {
        ...
        let _ = crate::repository::SqliteEntryRepository::new(uc).upsert_entry(&hub_slug, &entry);
...
pub async fn reindex(
    ...
    for entry in &disk_entries {
        match repo.upsert_entry(&slug, entry) {
            Ok(()) => indexed += 1,
