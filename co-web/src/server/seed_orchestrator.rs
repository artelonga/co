...
pub fn run_remote_sister_repo_seeds(config: &WebConfig) {
    ...
    for row in &rows {
        ...
        // CO-435: remote-git sync now goes through the `SourceAdapter` seam (the
        // git adapter wraps the same `crate::vcs` clone/pull). Adding a new
        // source kind = a new `impl SourceAdapter` + registry entry, with no
        // change here. See `crate::source`.
        //
        // CO-408: a credential-less HTTPS/SSH clone makes git emit
        // `fatal: could not read Username for 'https://github.com'` on the prod
        // machine — pure log noise, and the sync would fail anyway. The git
        // adapter reports `SkippedNoCredentials` so we skip quietly (info, not
        // fatal). The 15-min worker retries, so configuring CO_GIT_TOKEN later
        // recovers without a redeploy.
        let git = crate::source::GitSourceAdapter;
        match git.sync_blocking(remote_url, ref_name, &dest) {
            Ok(crate::source::SourceSyncOutcome::SkippedNoCredentials) => {
                tracing::info!(
