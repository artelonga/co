...
async fn create_flag_handler(
    ...
    match state.storage.lock() {
        Ok(storage) => match crate::ab::create_flag(storage.conn(), &body) {
            Ok(true) => (
                StatusCode::CREATED,
...
async fn list_flags_handler(
    ...
    match state.storage.lock() {
        Ok(storage) => match crate::ab::list_flags(storage.conn()) {
...
async fn toggle_flag_handler(
    ...
    match state.storage.lock() {
        Ok(storage) => match crate::ab::toggle_flag(storage.conn(), &key, body.enabled) {
            Ok(true) => {
