    ...
    c[3] = conn
        .query_row(
            "SELECT COUNT(DISTINCT email) FROM ( \
               ...
               WHERE email IS NOT NULL AND is_synthetic = 0 \
...
mod tests {
    ...
    fn insert_synthetic_lead(conn: &Connection, email: &str, status: &str, day: &str) {
        conn.execute(
            "INSERT INTO leads (created_at, updated_at, email, status, is_synthetic) \
             VALUES (?1, ?1, ?2, ?3, 1)",
            rusqlite::params![format!("{day} 09:00:00"), email, status],
        )
        .unwrap();
    ...
    /// CO-401: synthetic (staging-fixture) leads are excluded from the funnel
    /// rollup — Capture (step 4) and Qualify (step 5) count only real leads.
    #[test]
    fn test_synthetic_leads_excluded_from_funnel() {
        let conn = create_test_db();
        // One real lead + one synthetic, both 'triaged' (counts in steps 4 & 5).
        insert_lead(&conn, "real@example.com", "triaged", "2026-06-01");
        insert_synthetic_lead(&conn, "fixture@staging.local", "triaged", "2026-06-01");
        ...
        // Step 4 (Capture): distinct emails — only the real lead.
        assert_eq!(r.steps[3].count, 1, "synthetic lead excluded from Capture");
        // Step 5 (Qualify): triaged+ leads — only the real lead.
