...
fn test_schema_version_tracking() {
    ...
    assert_eq!(storage.schema_version(), 26); // CO-77 added v25 (last migration)
    assert_eq!(storage.schema_version(), 26); // CO-71 added v24

    // Creating a second Storage instance on same dir should not re-run migrations
    drop(storage);
    let storage2 = Storage::new(dir.path());
    assert_eq!(storage2.schema_version(), 26); // CO-77 added v25 (last migration)
    assert_eq!(storage2.schema_version(), 26); // CO-71 added v24
}
