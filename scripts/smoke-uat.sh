#!/usr/bin/env bash
# smoke-uat.sh — post-deploy regression smoke test for UAT.
#
# Usage:
#   bash scripts/smoke-uat.sh
#   BASE_URL=https://co-artelonga-uat.fly.dev bash scripts/smoke-uat.sh
#
# Run after every UAT deploy; gate prod deploy on exit 0.
# Exits 0 on full pass; exits 1 on any failure.
# Output is grep-friendly: each line starts with ✓ or ✗.
set -uo pipefail

BASE_URL="${BASE_URL:-https://co-artelonga-uat.fly.dev}"
...
EXPECTED_SW_CACHE_NAME='co-v4-offline'
